...
- NGINX Unit は基本的なWebサーバやProxy機能を持つが、 ``アプリケーションサーバ`` の位置づけであるため、HTTPをベースにした外部との接続とプログラムの安定実行に強みを持つ
- Apacheは Worker Process の fork によるメモリ増加、及びWorker Processを超えるクライアントからの接続があった場合に、C10K問題に見られるような限界が発生する可能性がある
